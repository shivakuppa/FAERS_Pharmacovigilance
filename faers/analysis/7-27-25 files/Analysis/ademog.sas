...
data Demo1 (DROP=VAR24);
	...
	ELSE If D_AGE>=0 AND D_AGE<=25 THEN D_AGEGRP="GROUP 1";
	ELSE If D_AGE>25 AND D_AGE<=50 THEN D_AGEGRP="GROUP 2";
	ELSE If D_AGE>50 AND D_AGE<=75 THEN D_AGEGRP="GROUP 3";
	ELSE If D_AGE>=76 THEN D_AGEGRP="GROUP 4";

	format AGE_COD $AGE_COD. GNDR_COD $GNDR. REPT_COD $REPT. WT_COD $WT. OCCP_COD $OCCP. I_F_COD $IFCOD.;
